...
std::string FrequencyTableCreatorFromTableGridExporter::CreateFrequencyJson(CTblGrid& table_grid)
{
    m_titles.reset();
...
void FrequencyTableCreatorFromTableGridExporter::WriteTitle(_tostream& /*os*/, const CFmt& /*fmt*/, const CString& sTitle)
{
    ...
    m_titles = std::make_unique<std::vector<std::string>>();

    SO::ForeachLine(UTF8_TODO::GetUtf8(sTitle), true,
        [&](const std::string_view title_sv)
        {
...
void FrequencyTableCreatorFromTableGridExporter::WriteCell(_tostream& /*os*/, int iCol, int iRow, const CFmt& /*fmt*/, const CString& sCellData, const CJoinRegion& join, const CArray<CJoinRegion>& /*aColHeaders*/)
{
    ...
    if( iRow == 0 )
    {
        ...
        if( iCol == 0 )
        {
            m_frequencyTable->symbol_name = UTF8_TODO::GetUtf8(sCellData);
        }
    ...
    if( iCol == 0 )
    {
        ...
        else
        {
            ...
            m_currentFrequencyRow->display_label = UTF8_TODO::GetUtf8(sCellData);
