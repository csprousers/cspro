...
CSPro::Dictionary::SimpleDictionaryCreator::SimpleDictionaryCreator(System::String^ name_prefix_, const bool decimal_char_default, const bool zero_fill_default)
    :   m_dictionary(new CDataDict()),
        m_nextStartPos(1)
{
    const std::string name_prefix = clr_helpers::to_string(name_prefix_);
    CString label_prefix = GetTitleCaseLabel(name_prefix_);

    m_dictionary->SetName(name_prefix + "_DICT");
    ...
    m_dictionary->SetAllowDataManagerModifications(true);
    m_dictionary->SetAllowExport(true);

    DictLevel dict_level;
    dict_level.SetName(name_prefix + "_LEVEL");
    dict_level.SetLabel(label_prefix + _T(" Level"));

    CDictRecord dict_record;
    dict_record.SetName(name_prefix + "_REC");
...
{
    CDictItem dict_item;
    dict_item.SetName(clr_helpers::to_string(name));
...
void CSPro::Dictionary::SimpleDictionaryCreator::Save(System::String^ file_path)
{
    try
    {
        m_dictionary->Save(clr_helpers::to_string(file_path));
    }
