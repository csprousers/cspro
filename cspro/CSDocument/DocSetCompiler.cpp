...
void DocSetCompiler::AddErrorOrWarning(const bool error, const std::string& text)
{
    ...
    // when errors occur in a file other than the one being compiled, report the error with the file path
    const std::string& file_path_with_error = !m_componentTextFilePaths.empty() ? m_componentTextFilePaths.back() :
                                                                                  SO::Empty_string;

    if( std::holds_alternative<ThrowErrors>(m_errorIssuer) )
    {
        throw CSProExceptionWithFilePath(file_path_with_error, text);
    }

    else if( error )
    {
        std::get<BuildWnd*>(m_errorIssuer)->AddError(file_path_with_error, text);
    }

    else
    {
        std::get<BuildWnd*>(m_errorIssuer)->AddWarning(file_path_with_error, text);
    }
}


JsonNode DocSetCompiler::ComponentText::GetJsonNode()
{
    try
    {
    ...
    catch( const JsonParseException& json_parse_exception )
    {
        ...
        throw CSProExceptionWithFilePath(file_path_holder.GetValue(), json_parse_exception.what());
    }
}
...
template<typename T>
T DocSetCompiler::GetFileTextOrModifiedIteration(const std::string& file_path)
{
    // try to use an open document
    if( !m_getFileTextOrModifiedIterationCallbacks.empty() )
    {
        const std::tuple<SharableString, int64_t>* const text_and_modified_iteration = m_getFileTextOrModifiedIterationCallbacks.back()(file_path);
    ...
    else
    {
        ...
        if( WindowsDesktopMessage::Send(UWM::Designer::FindOpenTextSourceEditable, &file_path, &text_source) == 1 )
        {
            ...
            {
                return text_source->GetTextAsSharableString();
            }
        }
    }
    ...
    if constexpr(std::is_same_v<T, int64_t>)
    {
        return PortableFunctions::FileModifiedTime(file_path);
    }

    else
    {
        // ... allowing FileIO to throw exceptions instead of using AddError because this is a fatal error
        return FileIO::ReadText(file_path);
    }
}


DocSetCompiler::ComponentText DocSetCompiler::GetComponentText(const std::string& file_path)
{
    return ComponentText
    {
        RAII::PushOnVectorAndPopOnDestruction<std::string>(m_componentTextFilePaths, file_path),
        GetFileTextOrModifiedIteration<SharableString>(file_path),
        JsonReaderInterface(PortableFunctions::PathGetDirectory(file_path))
    };
}


bool DocSetCompiler::JsonNodeContainsAndIsNotNull(const JsonNode& json_node, const std::string_view key_sv)
{
...
bool DocSetCompiler::EnsureJsonNodeIsObject(const JsonNode& json_node, const char* const type)
{
    if( json_node.IsObject() )
        return true;

    AddError("'%s' must be specified as a JSON object.", type);

    return false;
}


bool DocSetCompiler::EnsureJsonNodeIsArray(const JsonNode& json_node, const char* const type)
{
    if( json_node.IsArray() )
        return true;

    AddError("'%s' must be specified as a JSON array.", type);

    return false;
}


std::string DocSetCompiler::JsonNodeGetStringWithWhitespaceCheck(const JsonNode& json_node, const char* const key)
{
    std::string text = json_node.Get<std::string>(key);

    if( SO::IsWhitespace(text) )
        AddError("'%s' cannot be %s.", key, text.empty() ? "blank" : "whitespace");

    return text;
}


std::shared_ptr<DocSetComponent> DocSetCompiler::JsonNodeGetDocumentWithSupportForFilenameOnly(const JsonNode& json_node, DocSetSpec& doc_set_spec,
                                                                                               const bool create_component_if_not_found, const char* const document_type_for_error)
{
    // first search by absolute path
    std::string absolute_path = json_node.GetAbsolutePath();
    ...
    const std::string filename_only = json_node.Get<std::string>();
    const std::vector<std::shared_ptr<DocSetComponent>>* const doc_set_components_with_name = doc_set_spec.FindDocument(filename_only);

    if( doc_set_components_with_name != nullptr )
    {
        if( doc_set_components_with_name->size() != 1 )
        {
            AddError("The path '%s' is ambiguous. It could refer to '%s' or '%s'.",
                     filename_only.c_str(),
                     doc_set_components_with_name->front()->file_path.c_str(),
                     doc_set_components_with_name->at(1)->file_path.c_str());
            return nullptr;
        }
    ...
    if( create_component_if_not_found && PortableFunctions::FileIsRegular(absolute_path) )
        return std::make_unique<DocSetComponent>(DocSetComponent::Type::Document, std::move(absolute_path));

    if( document_type_for_error != nullptr )
        AddError("The %s path is not part of the Document Set: %s", document_type_for_error, absolute_path.c_str());
...
void DocSetCompiler::JsonNodeForeachNode(const JsonNode& json_node, const char* const type, const bool nodes_cannot_be_arrays_or_objects, const CF callback_function)
{
    ...
    json_node.ForeachNode(
        [&](const std::string_view key_sv, const JsonNode& attribute_value_node)
        {
            ...
            {
                AddError("'%s' (part of '%s') cannot be specified as an array or object.", key.c_str(), type);
                return;
            }
...
bool DocSetCompiler::CheckIfComponentsDoesNotContain(const DocSetSpec& doc_set_spec, const DocSetComponent::Type doc_set_component_type)
{
    ...
    {
        AddError("You cannot specify a '%s' when one has already been imported.", ToString(doc_set_component_type));
        return false;
    }
...
bool DocSetCompiler::CheckFileValidity(const DocSetComponent& doc_set_component, const bool error_if_file_does_not_exist/* = true*/)
{
    if( !PortableFunctions::FileIsRegular(doc_set_component.file_path) )
    {
        AddErrorOrWarning(error_if_file_does_not_exist, FormatText("The '%s' file does not exist: %s",
                                                                   ToString(doc_set_component.type), doc_set_component.file_path.c_str()));
        return false;
    }

    const std::string extension = PortableFunctions::PathGetFileExtension(doc_set_component.file_path);
    ...
    if( doc_set_component.type == DocSetComponent::Type::Document )
    {
        ...
        {
            AddError("The '%s' file must have the extension '%s': %s",
                     ToString(doc_set_component.type), FileExtensions::CSDocument, doc_set_component.file_path.c_str());
            return false;
        }
    }
    ...
    {
        AddError("The '%s' file cannot have the extension '%s': %s",
                 ToString(doc_set_component.type), extension.c_str(), doc_set_component.file_path.c_str());
        return false;
    }
...
bool DocSetCompiler::CheckIfDirectoryExists(const char* const type, const std::string& path)
{
    if( PortableFunctions::FileIsDirectory(path) )
        return true;

    AddError("The %s directory does not exist: %s", type, path.c_str());

    return false;
}


bool DocSetCompiler::CheckIfOverridesWithNewValue(const char* const type, const std::string& new_value, const std::string& old_value)
{
    ...
    if( !old_value.empty() )
    {
        AddWarning("The %s has already been specified but this value ('%s') will override the previous value ('%s').",
...
bool DocSetCompiler::AddComponent(DocSetSpec& doc_set_spec, std::shared_ptr<DocSetComponent> doc_set_component, const bool error_if_file_does_not_exist/* = true*/)
{
    ...
    const std::shared_ptr<const DocSetComponent> already_added_doc_set_component = doc_set_spec.FindComponent(doc_set_component->file_path, false);
    ...
    else if( already_added_doc_set_component->type == doc_set_component->type )
    {
        ...
        if( doc_set_component->type != DocSetComponent::Type::Document )
        {
            AddWarning("The '%s' file has already been added and will be ignored: %s",
                       ToString(doc_set_component->type), doc_set_component->file_path.c_str());
        }
    }
    ...
    {
        AddError("The '%s' file has already been added as a '%s' file: %s",
                 ToString(doc_set_component->type), ToString(already_added_doc_set_component->type), doc_set_component->file_path.c_str());
    }
...
void DocSetCompiler::CompileSpec(DocSetSpec& doc_set_spec, const JsonNode& json_node, const SpecCompilationType spec_compilation_type)
{
    ...
    {
        const JsonNode import_node = json_node.Get(JK::import);
        ...
        if( EnsureJsonNodeIsObject(import_node, JK::import) )
        {
            ...
            if( ( compiling_component_listing || compile_external_toc ) && JsonNodeContainsAndIsNotNull(import_node, JK::tableOfContents) )
            {
                if( CheckIfComponentsDoesNotContain(doc_set_spec, DocSetComponent::Type::TableOfContents) )
                    AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::TableOfContents, import_node.GetAbsolutePath(JK::tableOfContents)));
            }
            ...
            if( ( compiling_component_listing || compile_external_index ) && JsonNodeContainsAndIsNotNull(import_node, JK::index) )
            {
                if( CheckIfComponentsDoesNotContain(doc_set_spec, DocSetComponent::Type::Index) )
                    AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::Index, import_node.GetAbsolutePath(JK::index)));
            }

            // imported settings
            if( ( compiling_component_listing || compile_external_settings ) && import_node.Contains(JK::settings) )
            {
                for( const JsonNode& settings_node : import_node.GetArray(JK::settings) )
                    AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::Settings, settings_node.GetAbsolutePath()));
            }

            // imported definitions
            if( ( compiling_component_listing || compile_external_definitions ) && import_node.Contains(JK::definitions) )
            {
                for( const JsonNode& definitions_node : import_node.GetArray(JK::definitions) )
                    AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::Definitions, definitions_node.GetAbsolutePath()));
            }

            // imported context IDs
            if( ( compiling_component_listing || compile_context_ids ) && import_node.Contains(JK::contextIds) )
            {
                for( const JsonNode& context_ids_node : import_node.GetArray(JK::contextIds) )
    ...
    for( DocSetComponent& doc_set_component : VI_V(doc_set_spec.m_components) )
    {
        if( doc_set_component.type == DocSetComponent::Type::Settings )
        {
            if( compile_external_settings )
                CompileSettings(GetComponentText(doc_set_component.file_path).GetJsonNode(), doc_set_spec.m_settings, false);
        }

        else if( doc_set_component.type == DocSetComponent::Type::Definitions )
        {
            if( compile_external_definitions )
                CompileDefinitions(GetComponentText(doc_set_component.file_path).GetJsonNode(), doc_set_spec.m_definitions);
        }

        else if( doc_set_component.type == DocSetComponent::Type::ContextIds )
        {
            if( compile_context_ids )
                CompileContextIds(GetComponentText(doc_set_component.file_path).text.GetString(), doc_set_spec.m_contextIds);
        }
    }
    ...
    {
        for( const JsonNode& documents_node : json_node.GetArray(JK::documents) )
    ...
    for( size_t i = 0; i < doc_set_spec.m_components.size(); ++i )
    {
        ...
        if( doc_set_component.type == DocSetComponent::Type::TableOfContents )
        {
            if( compile_external_toc )
            {
                ...
                doc_set_spec.m_tableOfContents = CompileTableOfContents(doc_set_spec, GetComponentText(doc_set_component.file_path).GetJsonNode(), validate_toc_index_titles);
            }
        }
        ...
        else if( doc_set_component.type == DocSetComponent::Type::Index )
        {
            if( compile_external_index )
            {
                ...
                doc_set_spec.m_index = CompileIndex(doc_set_spec, GetComponentText(doc_set_component.file_path).GetJsonNode(), validate_toc_index_titles);
            }
        }
    }
    ...
    {
        doc_set_spec.m_coverPageDocument = JsonNodeGetDocumentWithSupportForFilenameOnly(json_node.Get(JK::coverPageDocument), doc_set_spec, true, "cover page document");
    ...
    if( compile_documents_node && json_node.Contains(JK::defaultDocument) )
        doc_set_spec.m_defaultDocument = JsonNodeGetDocumentWithSupportForFilenameOnly(json_node.Get(JK::defaultDocument), doc_set_spec, false, "default document");
...
void DocSetCompiler::CompileSpec(DocSetSpec& doc_set_spec, const std::string& text, const SpecCompilationType spec_compilation_type)
{
    JsonReaderInterface json_reader_interface(PortableFunctions::PathGetDirectory(doc_set_spec.GetFilePath()));
    CompileSpec(doc_set_spec, Json::Parse(text, &json_reader_interface), spec_compilation_type);
}


void DocSetCompiler::CompileSpecIfNecessary(DocSetSpec& doc_set_spec, const SpecCompilationType spec_compilation_type)
{
    if( !SpecRequiresCompilation(doc_set_spec, spec_compilation_type) )
        return;

    const SharableString text = GetFileTextOrModifiedIteration<SharableString>(doc_set_spec.GetFilePath());
...
bool DocSetCompiler::SpecRequiresCompilation(DocSetSpec& doc_set_spec, const SpecCompilationType spec_compilation_type) const
{
    ...
    for( const DocSetComponent& doc_set_component : VI_V(doc_set_spec.m_components) )
    {
        ...
        if( compile_if_modified && std::get<2>(*doc_set_spec.m_lastCompilationDetails) < DocSetCompiler::GetFileTextOrModifiedIteration<int64_t>(doc_set_component.file_path) )
            return true;
    }
...
std::tuple<std::string, bool> DocSetCompiler::GetPathWithRecursiveOption(const JsonNode& json_node)
{
    if( json_node.IsObject() )
    {
        return std::make_tuple(json_node.GetAbsolutePath(JK::path),
                               json_node.GetOrDefault(JK::recursive, false));
    }

    else
    {
        return std::make_tuple(json_node.GetAbsolutePath(), false);
    }
}


void DocSetCompiler::CompileDocumentsNode(DocSetSpec& doc_set_spec, const JsonNode& json_node)
{
    ...
    if( PortableFunctions::FileIsDirectory(path) )
    {
        ...
        directory_lister.SetNameFilter(FileExtensions::CreateWildcard(FileExtensions::CSDocument));

        for( std::string& file_path : directory_lister.GetPaths(path) )
            AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::Document, std::move(file_path), true));
    }

    else
    {
        AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::Document, std::move(path), true));
    }
}


void DocSetCompiler::ProcessContextIdsNode(DocSetSpec& doc_set_spec, const JsonNode& json_node)
{
    std::string file_path = json_node.GetAbsolutePath();
    ...
    if( !PortableFunctions::FileIsRegular(file_path) )
    {
        ...
        {
            std::string test_file_path = Path::Combine(m_globalSettings->cspro_code_path, PortableFunctions::PathToNativeSlash(json_node.Get<std::string>()));

            if( PortableFunctions::FileIsRegular(test_file_path) )
                file_path = std::move(test_file_path);
        }
    }

    AddComponent(doc_set_spec, std::make_unique<DocSetComponent>(DocSetComponent::Type::ContextIds, std::move(file_path)), false);
}


std::optional<DocSetTableOfContents> DocSetCompiler::CompileTableOfContents(DocSetSpec& doc_set_spec, const JsonNode& json_node, const bool validate_titles)
{
    return DocSetTableOfContents::Compile(*this, doc_set_spec, json_node, validate_titles);
}


std::optional<DocSetIndex> DocSetCompiler::CompileIndex(DocSetSpec& doc_set_spec, const JsonNode& json_node, const bool validate_titles)
{
    return DocSetIndex::Compile(*this, doc_set_spec, json_node, validate_titles);
}


void DocSetCompiler::CompileSettings(const JsonNode& json_node, DocSetSettings& doc_set_settings, const bool reset_settings)
{
    doc_set_settings.Compile(*this, json_node, reset_settings);
}


void DocSetCompiler::CompileDefinitions(const JsonNode& json_node, std::vector<std::tuple<std::string, std::string>>& definitions)
{
    JsonNodeForeachNode(json_node, ToString(DocSetComponent::Type::Definitions), true,
        [&](std::string key, const JsonNode& attribute_value_node)
        {
            ...
            std::string value = attribute_value_node.Get<std::string>();
            ...
            {
                AddWarning("The definition '%s' has already been specified but this value ('%s') will override the previous value ('%s').",
...
void DocSetCompiler::CompileContextIds(const std::string& resource_file_text, std::map<std::string, unsigned>& context_ids)
{
    const std::regex define_line_match_regex(R"(^\s*#define\s+([a-zA-Z]\S+)\s+(\d+|0[xX][0-9a-fA-F]+)\s*$)");

    SO::ForeachLine<std::string>(resource_file_text, false,
        [&](const std::string& line)
        {
            ...
            if( std::regex_match(line, matches, define_line_match_regex) )
            {
                ...
                const std::string id_text = matches.str(2);

                try
                {
                    const int base = SO::StartsWithNoCase(id_text, "0x") ? 16 : 10;
                    ...
                    {
                        AddWarning("The context ID '%s' has already been specified but this value ('%d') will override the previous value ('%d').",
                ...
                {
                    AddError("The context ID '%s' is not valid: %s", name.c_str(), id_text.c_str());
                }
            }
        });
}
...
DocSetSettings DocSetCompiler::GetSettingsFromSpecOrSettingsFile(const std::string& file_path, ErrorIssuerType error_issuer/* = ThrowErrors { }*/)
{
    const std::string file_text = FileIO::ReadText(file_path);

    DocSetCompiler doc_set_compiler(std::move(error_issuer));

    // parse the file as if it were a Document Set...
    if( SO::EqualsNoCase(PortableFunctions::PathGetFileExtension(file_text), FileExtensions::CSDocumentSet) )
    {
        DocSetSpec doc_set_spec(file_text);
    ...
    {
        JsonReaderInterface json_reader_interface(PortableFunctions::PathGetDirectory(file_text));
        const JsonNode json_node = Json::Parse(file_text, &json_reader_interface);
