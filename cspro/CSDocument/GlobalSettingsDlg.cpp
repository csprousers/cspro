    ...
    try
    {
        ...
        else
        {
            const int64_t seconds = CIMSAString::IsNumeric(m_automaticCompilationSeconds) ? CIMSAString::Val(m_automaticCompilationSeconds) :
                                                                                            -1;
            if( seconds < 0 )
                throw CSProException("The seconds value '%s' is not valid.", m_automaticCompilationSeconds.c_str());
        ...
        // validate the paths
        auto validate_path = [](const std::string& path, const bool is_directory)
        {
            if( !path.empty() && !( is_directory ? PortableFunctions::FileIsDirectory(path) :
                                                   PortableFunctions::FileIsRegular(path) ) )
            {
                throw CSProException("The path does not exist: %s", path.c_str());
            }
        };
...
void GlobalSettingsDlg::OnBrowseFile(std::string& path, const char* const path_type)
{
    UpdateData(TRUE);

    OpenFileDlg open_file_dlg(0, nullptr, path, L"Executables (*.exe)|*.exe|All Files (*.*)|*.*||", this);
    open_file_dlg.SetTitle(FormatText("Select the %s Executable", path_type));

    if( open_file_dlg.DoModal() != IDOK )
        return;

    path = open_file_dlg.GetFilePath();
...
{
    OnBrowseFile(m_globalSettings.html_help_compiler_path, "Microsoft HTML Help Compiler");
}


void GlobalSettingsDlg::OnBrowseWkhtmltopdf()
{
    OnBrowseFile(m_globalSettings.wkhtmltopdf_path, "wkhtmltopdf");
}
