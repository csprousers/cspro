...
bool DocSetTableOfContents::operator==(const DocSetTableOfContents& rhs) const
{
    ...
    const auto lhs_nodes_end = m_nodes.cend();
    ...
    for( ; lhs_nodes_itr != lhs_nodes_end; ++lhs_nodes_itr, ++rhs_nodes_itr )
    {
        ...
        if( std::holds_alternative<Chapter>(*lhs_nodes_itr) )
        {
            ...
            chapters_are_equal = [&](const Chapter& lhs_chapter, const Chapter& rhs_chapter)
            {
                ...
                const auto lhs_chapters_end = lhs_chapter.chapters.cend();
...
const DocSetComponent* DocSetTableOfContents::GetDocumentByPosition(const size_t position) const
{
...
public:
    ...
    std::optional<DocSetTableOfContents> Compile(const JsonNode& json_node);

private:
    void CompileProjectLink(std::string project);

    void CompileTableOfContentsNode(const JsonNode& json_node);

    std::optional<Chapter> CompileChapter(const JsonNode& json_node);
...
std::optional<DocSetTableOfContents> DocSetTableOfContents::CompileWorker::Compile(const JsonNode& json_node)
{
    ...
    {
        for( const JsonNode& toc_node : json_node.GetArray() )
        {
    ...
    const Chapter* const root_chapter = m_tableOfContents.GetRootChapter();

    if( root_chapter == nullptr )
    {
        m_docSetCompiler.AddError("A '%s' must define one node of titles, documents, and chapters.",
    ...
    {
        m_docSetCompiler.AddError("A '%s' must contain at least one document.",
                                  ToString(DocSetComponent::Type::TableOfContents));
        return std::nullopt;
    }
    ...
    auto add_documents_not_in_components = [&]()
    {
        for( auto documents_in_order_itr = documents_in_order_begin; documents_in_order_itr != documents_in_order_end; ++documents_in_order_itr )
        {
            ...
            if( m_docSetSpec.FindComponent(doc_set_component->file_path, false) == nullptr )
    ...
    try
    {
        ...
        for( const DocSetComponent& doc_set_component : VI_V(m_docSetSpec.GetComponents()) )
        {
            if( doc_set_component.type == DocSetComponent::Type::Document )
            {
                const auto& lookup = std::find_if(documents_in_order_begin, documents_in_order_end,
                    [&](const std::shared_ptr<DocSetComponent>& other_doc_set_component) { return SO::EqualsNoCase(doc_set_component.file_path, other_doc_set_component->file_path); });

                if( lookup == documents_in_order_end )
                {
                    m_docSetCompiler.AddError("The '%s' must contain the document specified in the 'documents' node: %s",
                                              ToString(DocSetComponent::Type::TableOfContents), doc_set_component.file_path.c_str());
                }
            }
        }
        ...
        for( auto documents_in_order_itr = documents_in_order_begin; documents_in_order_itr != documents_in_order_end; ++documents_in_order_itr )
        {
            ...
            {
                m_docSetCompiler.AddError("The '%s' cannot contain multiple instances of: %s",
                                          ToString(DocSetComponent::Type::TableOfContents), doc_set_component->file_path.c_str());
            }
        }
...
void DocSetTableOfContents::CompileWorker::CompileProjectLink(std::string project)
{
    try
    {
        // calling FindProjectDocSetSpecFilePath will throw an exception if the project name is not valid
        project = PortableFunctions::PathRemoveTrailingSlash(project);
        m_docSetSpec.GetSettings().FindProjectDocSetSpecFilePath(project);
        ASSERT(project.find_first_of(Path::SlashChars_sv) == std::string::npos);
        ...
        {
            throw CSProException("The '%s' cannot contain multiple links to the project: %s",
...
void DocSetTableOfContents::CompileWorker::CompileTableOfContentsNode(const JsonNode& json_node)
{
    ...
    {
        m_docSetCompiler.AddError("A '%s' can only have one node of titles, documents, and chapters.",
                                  ToString(DocSetComponent::Type::TableOfContents));
        return;
    }
...
std::optional<DocSetTableOfContents::Chapter> DocSetTableOfContents::CompileWorker::CompileChapter(const JsonNode& json_node)
{
    Chapter chapter;

    // title
    chapter.title = json_node.GetOrConstruct<std::string>(JK::title);

    if( chapter.title.empty() )
        m_docSetCompiler.AddError("A '%s' chapter must contain a title.", ToString(DocSetComponent::Type::TableOfContents));

    chapter.write_title_to_pdf = json_node.GetOrDefault(JK::writeTitleToPdf, true);

    // documents
    for( const JsonNode& documents_node : json_node.GetArrayOrEmpty(JK::documents) )
    {
        ...
        {
            m_tableOfContents.m_documentTitleOverrides.try_emplace(doc_set_component->file_path,
        ...
        {
            EnsureTitleExists(doc_set_component->file_path);
        }
    ...
    // chapters
    for( const JsonNode& chapters_node : json_node.GetArrayOrEmpty(JK::chapters) )
    {
    ...
    {
        m_docSetCompiler.AddError("A '%s' chapter must contain at least one document or chapter.",
                                  ToString(DocSetComponent::Type::TableOfContents));
        return std::nullopt;
    }
...
std::optional<DocSetTableOfContents> DocSetTableOfContents::Compile(DocSetCompiler& doc_set_compiler, DocSetSpec& doc_set_spec, const JsonNode& json_node, const bool validate_titles)
{
...
void DocSetTableOfContents::Writer::WriteChapter(const Chapter& chapter)
{
    ...
    if( !chapter.documents.empty() )
    {
        ...
        {
            const auto& title_override_lookup = m_documentTitleOverrides->find(doc_set_component.file_path);
            const std::string* const title_override = ( title_override_lookup != m_documentTitleOverrides->cend() ) ? &title_override_lookup->second :
                                                                                                                      nullptr;
            WriteDocument(doc_set_component.file_path, title_override);
        }
...
{
public:
    DocSetTableOfContents_JsonWriterWorker(JsonWriter& json_writer, DocSetSpec* const doc_set_spec, const bool write_documents_with_filename_only_when_possible, const bool write_evaluated_titles, const bool detailed_format)
...
protected:
    void StartWriting(const size_t num_root_nodes) override
    {
        if( num_root_nodes > 1 )
        {
    ...
    void WriteDocument(const std::string& csdoc_file_path, const std::string* const title_override) override
    {
...
void DocSetTableOfContents::WriteJson(JsonWriter& json_writer, DocSetSpec* const doc_set_spec/* = nullptr*/,
