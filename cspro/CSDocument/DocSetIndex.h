...
public:
    ...
    static std::optional<DocSetIndex> Compile(DocSetCompiler& doc_set_compiler, DocSetSpec& doc_set_spec, const JsonNode& json_node, bool validate_titles);
...
private:
    static bool ChmIndexTextLess(const std::string& text1, const std::string& text2);

    static std::vector<Entry> CompileEntries(DocSetCompiler& doc_set_compiler, DocSetSpec& doc_set_spec, const JsonNode& json_node, size_t entries_level);
...
protected:
    ...
    virtual void WriteEntry(const std::string& csdoc_file_path, const std::string* title_override, const void* subentries_tag) { csdoc_file_path; title_override; subentries_tag; }
