...
{
public:
    SQLiteQuestionnaireSerializer(UniqueId repository_id, sqlite3* db);
...
private:
    ...
    void BindCaseItem(SQLiteStatement& statement, int param_number, const CaseItem& case_item, const CaseItemIndex& index, int64_t revision) const;
    void BindBinaryCaseItem(SQLiteStatement& statement, int signature_param_number, int metadata_param_number,
                            const BinaryCaseItem& binary_case_item, const CaseItemIndex& index, int64_t revision) const;
    ...
    void ReadRecordItems(CaseRecord& case_record, CaseItemIndex& index, SQLiteStatement& select_statement, int start_column) const;

    void SetCaseItemValueStatement(const CaseItem& case_item, CaseItemIndex& index, SQLiteStatement& statement, int column_number) const;
    void SetBinaryCaseItemFromStatement(const BinaryCaseItem& binary_case_item, CaseItemIndex& index, SQLiteStatement& statement, int signature_column_number, int metadata_column_number) const;
    ...
    void DeleteCase(const std::string& case_id) const;
...
private:
    ...
    std::unique_ptr<SQLiteBinaryItemSerializer> m_binaryItemSerializer;
