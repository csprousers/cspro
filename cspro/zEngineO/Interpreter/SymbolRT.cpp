...
Engine::Value LogicInterpreter::ex_Symbol_getLabel(const int program_index)
{
    ...
    // otherwise we have to look for the label in the dictionary, which is
    // currently the only location where labels can have multiple languages
    ...
    if( engine_item_accessor != nullptr )
    {
        ...
        if( language_index.has_value() )
            return UTF8_TODO::GetUtf8(dict_item.GetLabelSet().GetLabel(*language_index, false));
    }
...
Engine::Value LogicInterpreter::ex_Symbol_getName(const int program_index)
{
    const auto& symbol_va_with_subscript_node = GetNode<Nodes::SymbolVariableArgumentsWithSubscript>(program_index);
    const Symbol& symbol = GetFromSymbolOrEngineItemForStaticFunction(symbol_va_with_subscript_node.symbol_index, symbol_va_with_subscript_node.subscript_compilation);
