...
public:
    ...
    CaseItemIndex(CaseItemIndex&&) = default;

    // the CaseRecord will be stored as a const object so that CaseItemIndex can be const, but
    // CaseRecord::GetCaseItemIndex also has a non-const version, so we will cast the CaseRecord as needed
    ...
    std::string GetFullOccurrencesText(const CaseItem& case_item) const;
    std::string GetMinimalOccurrencesText(const CaseItem& case_item) const;
    bool SetOccurrencesFromText(const CaseItem& case_item, std::string_view occurrences_text_sv);

    // helpers for serializing case item references to/from text
    std::string GetSerializableText(const CaseItem& case_item) const;
    static std::tuple<const CaseItem*, std::unique_ptr<CaseItemIndex>> FromSerializableText(const Case& data_case, std::string_view serializable_text_sv);
...
inline CaseItemIndex::CaseItemIndex(const CaseRecord& case_record, const size_t record_occurrence)
