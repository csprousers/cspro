...
// --------------------------------------------------------------------------
// DictionaryPre80SpecFileConverter
// --------------------------------------------------------------------------

class DictionaryPre80SpecFileConverter
{
public:
    DictionaryPre80SpecFileConverter(const InterfaceString file_path, JsonWriter& json_writer, std::vector<std::tuple<CString, CString, int>> lines)
        :   m_jsonWriter(json_writer),
            m_directory(PortableFunctions::PathGetDirectory(file_path.GetString<std::string>())),
...
private:
    ...
    {
        throw CSProException("Unknown command on line %d: %s", std::get<2>(m_lines[m_nextLine - 1]), UTF8_TODO::GetUtf8(m_command).c_str());
    }
...
void DictionaryPre80SpecFileConverter::ReadLanguages()
{
    ...
    while( GetLine() && !CommandIs(_T("[Level]")) )
    {
        if( CommandIs(_T("[Languages]")) )
        {
            ...
            while( GetLineIfNotSectionStart() )
                m_languages.emplace_back(UTF8_TODO::GetUtf8(m_command), UTF8_TODO::GetUtf8(m_argument));
...
bool DictionaryPre80SpecFileConverter::ProcessBase(bool named_entity, CString* name_read/* = nullptr*/)
{
...
void DictionaryPre80SpecFileConverter::WriteAliasesAndNotes(bool named_entity)
{
    if( named_entity && !m_aliasesStack.top().empty() )
        m_jsonWriter.Write(JK::aliases, m_aliasesStack.top());

    m_jsonWriter.WriteIfNotBlank(JK::note, UTF8_TODO::GetUtf8(m_notesStack.top()));
}


void DictionaryPre80SpecFileConverter::ProcessOccurrenceLabel(OccurrenceLabels& occurrence_labels, const std::optional<unsigned>& max_occurrences)
{
...
void DictionaryPre80SpecFileConverter::WriteOccurrencesNode(std::optional<bool> required, unsigned max_occurrences, const OccurrenceLabels& occurrence_labels)
{
...
void DictionaryPre80SpecFileConverter::ReadDictionary()
{
    ...
    if( version_before_74 )
    {
        ...
        m_jsonWriter.BeginObject(JK::security)
                    .Write(JK::settings, CDataDict::SerializeSecurityOptions(UTF8_TODO::GetUtf8(name), false, true, 0))
                    .EndObject();
    }
...
void DictionaryPre80SpecFileConverter::ReadLevels()
{
...
void DictionaryPre80SpecFileConverter::ReadRecords()
{
    for( bool read_more_entries = true; read_more_entries; )
    {
        m_jsonWriter.BeginObject();
        ...
        while( ( read_more_entries = GetLine() ) == true )
        {
            ...
            {
                m_jsonWriter.Write(JK::recordType, UTF8_TODO::GetUtf8(CIMSAString(m_argument).GetToken()));
            }
...
void DictionaryPre80SpecFileConverter::ReadItems()
{
    for( bool read_more_entries = true; read_more_entries; )
    {
        m_jsonWriter.BeginObject();
        ...
        while( ( read_more_entries = GetLine() ) == true )
        {
            ...
            {
                content = UTF8_TODO::GetWide(SO::TitleToCamelCase(UTF8_TODO::GetUtf8(m_argument)));
            }
            ...
            {
                capture_info.SetCaptureType(CaptureInfo::GetCaptureTypeFromSerializableName(UTF8_TODO::GetUtf8(m_argument)).value_or(CaptureType::Unspecified));
            }

            else if( CommandIs(L"CaptureDateFormat") && capture_info.GetCaptureType() == CaptureType::Date )
            {
                capture_info.GetExtended<DateCaptureInfo>().SetFormat(UTF8_TODO::GetUtf8(m_argument));
            }
...
void DictionaryPre80SpecFileConverter::ReadValueSets()
{
    for( bool read_more_entries = true; read_more_entries; )
    {
        m_jsonWriter.BeginObject();
...
void DictionaryPre80SpecFileConverter::ReadValues()
{
    for( bool read_more_entries = true; read_more_entries; )
    {
        m_jsonWriter.BeginObject();
        ...
        while( ( read_more_entries = GetLine() ) == true && !CommandIs(_T("Value")) )
        {
            ...
            else if( CommandIs(_T("Name")) )
            {
                ...
                ASSERT(SpecialValues::StringIsSpecial(UTF8_TODO::GetUtf8(special_value)));
                m_jsonWriter.Write(JK::special, special_value);
            }

            else if( CommandIs(_T("Image")) )
            {
                m_jsonWriter.Write(JK::image, MakeFullPath(m_directory, UTF8_TODO::GetUtf8(m_argument)));
            }
...
void DictionaryPre80SpecFileConverter::ReadRelations()
{
    ...
    do
    {
        ...
        m_jsonWriter.WriteObjects(JK::links, parts,
            [&](const Part& part)
            {
                m_jsonWriter.WriteIfNotBlank(JK::primaryLink, UTF8_TODO::GetUtf8(part.primary_link))
                            .WriteIfNotBlank(JK::secondary, UTF8_TODO::GetUtf8(part.secondary_name))
                            .WriteIfNotBlank(JK::secondaryLink, UTF8_TODO::GetUtf8(part.secondary_link));
            });
...
std::string CDataDict::ConvertPre80SpecFile(const InterfaceString file_path)
{
    ...
    if( !specfile.Open(file_path.GetString<std::wstring>().c_str(), CFile::modeRead) )
        throw CSProException("Failed to open the dictionary: %s", file_path.c_str_utf8());
    ...
    try
    {
        ...
        DictionaryPre80SpecFileConverter converter(file_path, *json_writer, std::move(lines));
        converter.Convert();
    ...
    {
        throw CSProException("There was an error reading the dictionary %s:\n\n%s",
