...
protected:
    // methods to be overridden by the SPSS/Stata writers
    virtual bool AddStringLabelSets() = 0;

    virtual std::string GetFixedWidthNumericFormat(const CDictItem& dict_item) = 0;
    virtual std::optional<std::string> GetFixedWidthStringFormat(unsigned width) = 0;
