...
public:
        ...
        :   m_finalizeStatementWhenComplete(true)
    {
        if( sqlite3_prepare_v2(db, sql, -1, &m_stmt, nullptr) != SQLITE_OK && throw_prepare_exception )
            throw SQLiteStatementException("SQLite: Could not prepare: %s", sql);
    }
    ...
    SQLiteStatement(sqlite3* db, const std::string& sql, const bool throw_prepare_exception = false)
        :   m_finalizeStatementWhenComplete(true)
    {
        if( sqlite3_prepare_v2(db, sql.c_str(), static_cast<int>(sql.length()) + 1, &m_stmt, nullptr) != SQLITE_OK && throw_prepare_exception )
            throw SQLiteStatementException("SQLite: Could not prepare: %s", sql.c_str());
    }
...
public:
    ...
    SQLiteStatement& Bind(PT parameter_number_or_name, const VT& value)
    {
        ...
        else if constexpr(std::is_same_v<VT, const wchar_t*> ||
                          std::is_same_v<VT, wstring_view>)
        {
            const std::string utf8_value = UTF8Convert::WideToUTF8(value);
            sqlite3_bind_text(m_stmt, pn, utf8_value.data(), utf8_value.length(), SQLITE_TRANSIENT);
        }

        else if constexpr(std::is_same_v<VT,
                          std::tuple<const std::byte*, size_t>>)
        {
    ...
    {
        return Bind(parameter_number_or_name, UTF8Convert::WideToUTF8(value));
    }
    ...
    void StepCheckResult(const int result)
    {
        if( sqlite3_step(m_stmt) != result )
            throw SQLiteStatementException("SQLite: Stepping did not result in code: %d", result);
    }
    ...
    std::string GetColumnName(const int column_num)
    {
        return sqlite3_column_name(m_stmt, column_num);
    }
    ...
    VT GetColumn(const int column_num)
    {
        ...
        {
            const char* const text = reinterpret_cast<const char*>(sqlite3_column_text(m_stmt, column_num));
            return ( text != nullptr ) ? std::string(text) :
                                         std::string();
        }

        else if constexpr(std::is_same_v<VT, std::wstring> ||
                          std::is_same_v<VT, CString>)
        {
            const char* const text = reinterpret_cast<const char*>(sqlite3_column_text(m_stmt, column_num));
        ...
        else if constexpr(std::is_same_v<VT, std::vector<std::byte>>)
        {
            ...
            const std::byte* const bytes = reinterpret_cast<const std::byte*>(sqlite3_column_blob(m_stmt, column_num));
