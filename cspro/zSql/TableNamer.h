...
namespace Sqlite
{
    ...
    std::string CreateValidTableName(std::string table_name);
}


inline std::string Sqlite::CreateValidTableName(std::string table_name)
{
    ...
    if( table_name.length() >= HashedCharactersSuffix_sv.length() )
    {
        ...
        if( SO::EqualsNoCase(HashedCharactersSuffix_sv, std::string_view(table_name.data(), invalid_suffix_pos)) )
        {
    ...
    if( invalid_characters_hash.has_value() )
    {
        table_name.append(IntToString(*invalid_characters_hash));
