...
void DeviceStateEvent::SetupTables(Log& log)
{
    log.CreateTable(ParadataTable::DeviceStateEvent)
            .AddColumn("bluetooth_enabled", Table::ColumnType::Boolean)
            .AddColumn("gps_enabled", Table::ColumnType::Boolean, true)
            .AddColumn("wifi_enabled", Table::ColumnType::Boolean)
            .AddColumn("wifi_ssid_text", Table::ColumnType::Long, true)
            .AddColumn("mobile_network_enabled", Table::ColumnType::Boolean, true)
            .AddColumn("mobile_network_type", Table::ColumnType::Text, true)
            .AddColumn("mobile_network_name_text", Table::ColumnType::Long, true)
            .AddColumn("mobile_network_strength", Table::ColumnType::Double, true)
            .AddColumn("battery_level", Table::ColumnType::Double, true)
            .AddColumn("battery_charging", Table::ColumnType::Boolean, true)
            .AddColumn("screen_brightness", Table::ColumnType::Double, true)
            .AddColumn("screen_orientation_portrait", Table::ColumnType::Boolean)
        ;
}
        ...
        if( WlanEnumInterfaces(hClientHandle, nullptr, &pIfList) == ERROR_SUCCESS )
        {
            for( DWORD i = 0; i < pIfList->dwNumberOfItems; ++i )
            {
                                       ...
                                       &pdwDataSize, (PVOID*)&pConnectionAttributes, nullptr) == ERROR_SUCCESS )
                {
                    ...
                    {
                        m_deviceState.wifi_ssid = TC::ToUtf8(pConnectionAttributes->strProfileName);
                        break;
                    }
    ...
    if( GetSystemPowerStatus(&system_power_status) )
    {
            ...
            ( system_power_status.BatteryFlag != 255 ) )  // unknown status
        {
            if( system_power_status.BatteryLifePercent != 255 )
                m_deviceState.battery_level = static_cast<double>(system_power_status.BatteryLifePercent);
    ...
    if( hMonitor != nullptr )
    {
        ...
        if( GetNumberOfPhysicalMonitorsFromHMONITOR(hMonitor, &dwPhysicalMonitors) == TRUE )
        {
            ...
            if( GetMonitorBrightness(hPhysicalMonitor, &dwMinimumBrightness, &dwCurrentBrightness, &dwMaximumBrightness) )
                m_deviceState.screen_brightness = static_cast<double>(dwCurrentBrightness);
