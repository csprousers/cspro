...
namespace Listing { class JsonLister; }


class Listing::JsonLister : public Lister
{
public:
    JsonLister(std::shared_ptr<ProcessSummary> process_summary, const std::string& file_path, std::shared_ptr<const CaseAccess> case_access);
    ~JsonLister();
...
private:
    ...
    std::unique_ptr<std::vector<std::string>> m_levelNames;

    std::vector<std::tuple<const CaseItem*, std::variant<double, SharableString>>> m_keyValues;
