...
namespace Logic { class BaseCompiler; class BaseCompilerSettings; class SymbolTable; }


class ZLOGICO_API Logic::BaseCompiler : public BasicTokenCompiler
{
...
public:
    ...
    void CheckIfValidNewSymbolName(std::string_view new_symbol_name_sv);
    ...
    template<typename T>
    size_t NextKeywordOrError(const T& keywords);
    size_t NextKeywordOrError(std::initializer_list<const char*> keywords) { return NextKeywordOrError(cs::span<const char* const>(keywords)); }
    ...
    bool NextKeywordIf(const char* keyword) { return ( NextKeyword({ keyword }) == 1 ); }
...
private:
    void CheckSymbolCase(const Symbol& symbol, const std::string& compiled_case);
...
template<typename T>
size_t Logic::BaseCompiler::NextKeywordOrError(const T& keywords)
{
    const size_t keyword_type = NextKeyword(keywords);

    if( keyword_type == 0 )
        IssueError(7016, SO::CreateSingleString(tcb::make_span(keywords)).c_str());
