...
//  ﾀﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾄﾁﾄﾄﾄﾄﾄﾄﾄﾁ= --------------------------------------------
    ...
    SYMBOLFN_SETVALUEFROMJSON_CODE          = 442, // 20221005
    ...
    // CSPro 8.1
    JSFN_EVAL_CODE                          = 456, // 20250113
    JSFN_INVOKE_CODE                        = 457, // 20250114
    JSFN_HASVALUE_CODE                      = 458, // 20250117
    JSFN_GETVALUEJSON_CODE                  = 459, // 20250117
    JSFN_SETVALUEFROMJSON_CODE              = 460, // 20250117
...
namespace Logic
{
    ...
    namespace FunctionTable
    {
        ...
        ZLOGICO_API bool IsFunctionNamespace(std::string_view text_sv, const std::variant<SymbolType, FunctionNamespace>& symbol_type_or_function_namespace,
        ...
        ZLOGICO_API bool IsFunction(std::string_view text_sv, const std::variant<SymbolType, FunctionNamespace>& symbol_type_or_function_namespace,
                                    const FunctionDetails** function_details = nullptr);

        // the extended version of IsFunction checks:
        // - if function_domain is AllSymbolsDomain, whether or not the function applies to ALL or SOME symbols (e.g., Symbol.setValueFromJson)
        // - if function_domain is SymbolType, whether or not the function applies to that symbol type (e.g., Image.getName)
        ZLOGICO_API bool IsFunctionExtended(std::string_view text_sv, const FunctionDomain& function_domain,
                                            const FunctionDetails** function_details = nullptr);

        // checks if the text is a function belonging to a symbol or to the symbol's wrapped type
        ZLOGICO_API bool IsFunction(std::string_view text_sv, const Symbol& symbol, const FunctionDetails** function_details = nullptr);

        ZLOGICO_API const FunctionDetails* GetFunctionDetails(FunctionCode function_code);

        ZLOGICO_API const char* GetFunctionName(FunctionCode function_code);
        ZLOGICO_API const char* GetFunctionNamespaceName(FunctionNamespace function_namespace);
    }
}
