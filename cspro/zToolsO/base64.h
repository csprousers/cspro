...
{
public:
    static std::string Encode(std::string_view text_sv)               { return base64_encode(reinterpret_cast<unsigned const char*>(text_sv.data()), uint32_cast(text_sv.length())); }
    static std::string Encode(const std::vector<std::byte>& contents) { return base64_encode(reinterpret_cast<unsigned const char*>(contents.data()), uint32_cast(contents.size())); }
