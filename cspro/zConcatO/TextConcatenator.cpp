                           ...
                           const ConnectionString& output_connection_string)
{
    ...
    {
        throw CSProException("The output of text concatenation must be to a text file, not to a file of type '%s'.",
    ...
    std::tie(file_sizes, total_file_size) = ConcatenatorHelpers::CalculateFileSizes(input_connection_strings, std::nullopt);

#ifdef WIN32
    // on Windows, see if there is enough room for this file
    const std::string volume = PathGetVolume(output_connection_string.GetFilePath());
    ...
    if( volume.length() == 3 && volume[1] == ':' )
    {
        ...
        GetDiskFreeSpace(TC::ToWide(volume).c_str(), &sectors_per_cluster, &bytes_per_second, &free_clusters, &total_clusters);
        ...
        {
            throw CSProException("The concatenated file requires at least %s bytes but only %s bytes are available.",
                                 IntToString(total_file_size).c_str(), IntToString(free_size).c_str());
        }
    }
#endif
    ...
    // concatenate each file to a temporary file
    TemporaryFile temporary_file;
    output_file.OpenForTextWritingCreate(temporary_file.GetPath());
    ...
    for( size_t i = 0; i < input_connection_strings.size(); ++i )
    {
        ...
        {
            concatenator_reporter.ErrorFileOpenFailed(input_connection_string.GetFilePath());
            continue;
        }

        // only text files can be concatenated
        if( !DataRepositoryHelpers::TypeWritesToText(input_connection_string.GetType()) )
        {
            concatenator_reporter.ErrorInvalidEncoding(input_connection_string.GetFilePath());
        ...
        {
            GetFileBOM(input_connection_string.GetFilePath(), encoding);
        ...
        else if( encoding == Encoding::Utf8 )
        {
            ...
            bom_size = TextEncoding::Utf8Bom_sv.length();
        }

        else // if( encoding == Encoding::Utf16LE )
        {
            // every UTF-16 character will map to at most four UTF-8 bytes
            conversion_buffer_size_needed = BufferSize * TC::MaxUtf8BytesNeededForWideChar();
            bom_size = 2;
        }
        ...
        {
            FileIO::File input_file; // TEXT_ENCODING_TODO use TextFile and its WriteString method instead so that encoding conversions occur automatically

            try
            {
                input_file.OpenForReading(input_connection_string.GetFilePath());
            }
            ...
            concatenator_reporter.SetSource(ConcatenatorHelpers::GetReporterSourceText("File", input_connection_strings, i));

            // skip past the BOM
            if( bom_size != 0 )
                input_file.Seek(bom_size, SEEK_SET);
            ...
            while( remaining_bytes > 0 )
            {
                ...
                {
                    bytes_read = input_file.ReadExact(read_buffer.data(), BufferSize);
                }

                else
                {
                    bytes_read = input_file.Read(read_buffer.data(), static_cast<size_t>(remaining_bytes));
    ...
    temporary_file.Rename(output_connection_string.GetFilePath());
}
