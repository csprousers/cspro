 ...
 * task to be canceled if the task is canceled or the timeout passes.
 *
 */
public class CancelableTaskRunner {
    ...
    public interface CancelChecker {
        ...
        /**
         * Return true if task execution has been canceled.
         */
        boolean isCanceled();
    }
    ...
    public <T> T run(final ExecutorService threadPool, final CancelableTask<T> task, final CancelChecker cancelChecker, int timeout, TimeUnit timeoutUnit) throws ExecutionException
    {
        ...
        while ((cancelChecker == null || !cancelChecker.isCanceled()) &&
        ...
        if (!futureTask.isDone()) {
            ...
            Timber.d("Aborting CancelableTask after " + (System.nanoTime() / 1e9) + " seconds and cancel = " + cancelChecker.isCanceled());
