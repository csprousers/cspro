...
{
public:
    // Looks at the specified input format (autodetect if not specified) and resolves it
    // (BinaryEncodingInput -> BinaryEncodingResolvedInput).
    ...
    // Looks at the specified input format (autodetect if not specified) and returns a
    // non-null pointer to the converted bytes, potentially throwing an exception on a conversion error.
...
{
public:
    // Looks at the specific output format (data URL if not specified) and
    // creates an object to convert bytes to that format.
...
private:
    ...
    std::string ConvertImmediately(const std::vector<std::byte>& bytes, std::string_view mime_type_sv);
    std::string ConvertCache(std::shared_ptr<const std::vector<std::byte>> bytes, std::string mime_type);
...
std::string BytesToStringConverter::Convert(BT&& bytes, MT&& mime_type)
{
    ...
    {
        case BinaryEncodingOutput::Cache:
            return ConvertCache(GetSharedPointerFromBytes(std::forward<BT>(bytes)), std::forward<MT>(mime_type));

        case BinaryEncodingOutput::LocalhostUrl:
            return ConvertLocalhost(GetSharedPointerFromBytes(std::forward<BT>(bytes)), std::forward<MT>(mime_type));

        default:
            return ConvertImmediately(*GetPointer(bytes), std::forward<MT>(mime_type));
    }
}
