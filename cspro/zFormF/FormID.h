...
public:
    FormReportID(CFormDoc* pDoc, std::optional<ReportFile> report_file);

    const ReportFile& GetReportFile() const { ASSERT(m_reportFile.has_value()); return *m_reportFile; }
...
private:
    std::optional<ReportFile> m_reportFile;
};
